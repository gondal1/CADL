...
def encoder(x, phase_train, dimensions=[], filter_sizes=[], convolutional=False, activation=tf.nn.relu, output_activation=tf.nn.sigmoid, reuse=None):
    ...
    for layer_i, n_output in enumerate(dimensions):
        with tf.variable_scope(str(layer_i), reuse=reuse):
            if convolutional:
                h, W = conv2d(x=current_input, n_output=n_output, k_h=filter_sizes[layer_i], k_w=filter_sizes[layer_i], padding='SAME', reuse=reuse)
            else:
                h, W = linear(x=current_input, n_output=n_output, reuse=reuse)
            #norm = batch_norm(x=h, phase_train=phase_train, name='bn', reuse=reuse)   # For now not doing batch norm here
            output = activation(h)
...
def generator(z, phase_train, output_h, output_w, convolutional=True, n_features=32, rgb=False, reuse=None):
...
def discriminator(x, phase_train, convolutional=True, n_features=32, rgb=False, reuse=None):
    """Summary
    ...
    with tf.variable_scope('discriminator', reuse=reuse):
        return encoder(x=x, phase_train=phase_train, convolutional=convolutional, filter_sizes=[5, 5, 5, 5], dimensions=[n_features, n_features * 2, n_features * 4, n_features * 8] if convolutional else [n_features, 128, 256], 
		activation=tf.nn.relu6, output_activation=None, reuse=reuse)
...
def GAN(input_shape, n_latent, n_features, rgb, debug=True):
    ...
    D_real_logits = discriminator(x, phase_train, n_features=n_features, rgb=rgb)
    ...
    G = generator(z, phase_train, output_h=input_shape[1], output_w=input_shape[2], n_features=n_features, rgb=rgb)
    sum_G = tf.image_summary("G", G)

    # Discriminator for generated samples
    D_fake_logits = discriminator(G, phase_train, n_features=n_features, rgb=rgb, reuse=True)
...
def train_ds():
    ...
    from dataset_utils import create_input_pipeline
    ...
    [d_vari.append((v.name)) for v in tf.trainable_variables() if v.name.startswith('discriminator')]
    print d_vari
    vars_g = [v for v in tf.trainable_variables()
              if v.name.startswith('generator')]
    print('Training generator variables:')
    [g_vari.append(v.name) for v in tf.trainable_variables() if v.name.startswith('generator')]
    print g_vari
